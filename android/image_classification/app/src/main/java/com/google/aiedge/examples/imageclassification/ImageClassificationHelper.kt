...
class ImageClassificationHelper(
    ...
    companion object {
        ...
        val DEFAULT_MODEL = Model.MetalMobileNetV2
    ...
    suspend fun initClassifier() {
        interpreter = try {
            ...
            labels = listOf("Crazing", "Inclusion", "Patches", "Pitted", "Rolled", "Scratches") //getModelMetadata(litertBuffer)
    ...
    suspend fun classify(bitmap: Bitmap, rotationDegrees: Int) {
        try {
            withContext(Dispatchers.IO) {
               ...
               // val categories = MutableList(outputList.size) { index -> Category("" + index, outputList[index]) }
    ...
    enum class Model(val fileName: String) {
      MetalMobileNetV2("metal_defect_model.tflite")
    }
